...
public class Equal extends StableFunction {
	...
	private static StringBuilder buildRelationExpr(FelNode node, FelContext ctx, String operator) {
		...
		/*
		 * if(ReflectUtil.isPrimitiveNumber(leftType)&&ReflectUtil.isPrimitiveNumber(leftType)){ //如果左右都是基本数值类型，直接==运算就行了。 // 实践证明，这种处理方式不行
		 * sb.append(left).append(operator).append(right); }else if (Number.class.isAssignableFrom(leftType)) { sb.append(left);
		 * sb.append(operator); appendNumber(rightType, right, sb); } else if (Number.class.isAssignableFrom(rightType)) {
		 * appendNumber(leftType, left, sb); sb.append(operator); sb.append(right); } else if (Boolean.class.isAssignableFrom(leftType)) {
		 * sb.append(left); sb.append(operator); appendBoolean(rightType, right, sb); } else if (Boolean.class.isAssignableFrom(rightType))
		 * { appendBoolean(leftType, left, sb); sb.append(operator); sb.append(right); } else if (String.class.isAssignableFrom(leftType)) {
		 * sb.append("StringUtils.equals(" + left + ",ObjectUtils.toString(" + right + "))"); } else if
		 * (String.class.isAssignableFrom(rightType)) { sb.append("StringUtils.equals(ObjectUtils.toString(" + right + ")," + left + ")"); }
		 * return sb;
		 */
	}

	/*
	 * static public void appendNumber(Class<?> type, String src, StringBuilder sb) { // 当type是基本类型时，会出现问题 if
	 * (ReflectUtil.isPrimitiveOrWrapNumber(type)) { // if (Number.class.isAssignableFrom(type)) { // 当type是数值型时，使用toString sb.append(src);
	 * } else if (String.class.isAssignableFrom(type)) { // 当type是字符型时，转成double型 sb.append("NumberUtil.toDouble(" + src + ")"); } else
	 * if(Null.class.isAssignableFrom(type)){ sb.append("Double.MIN_VALUE"); }else{ // 当type是Object时，this.equals // FIXME } }
	 */

	/*
	 * static public void appendBoolean(Class<?> type, String src, StringBuilder sb) { if (Boolean.class.isAssignableFrom(type)) { //
