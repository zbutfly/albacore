...
public final class RamUsageEstimator {
	...
	public static enum JvmFeature {
		OBJECT_REFERENCE_SIZE("Object reference size estimated using array index scale."), ARRAY_HEADER_SIZE(
				"Array header size estimated using array based offset."), FIELD_OFFSETS(
						"Shallow instance size based on field offsets."), OBJECT_ALIGNMENT(
								"Object alignment retrieved from HotSpotDiagnostic MX bean.");
	...
	/**
	 * A constant specifying the object alignment boundary inside the JVM. Objects will always take a full multiple of this constant,
	...
	static {
		...
		try {
			...
			try {
				...
				try {
					...
					hotSpotBean = ManagementFactory.newPlatformMXBeanProxy(ManagementFactory.getPlatformMBeanServer(),
		...
		JVM_INFO_STRING = "[JVM: " + Constants.JVM_NAME + ", " + Constants.JVM_VERSION + ", " + Constants.JVM_VENDOR + ", "
	...
	/**
	 * Returns true, if the current JVM is fully supported by {@code RamUsageEstimator}. If this method returns {@code false} you are maybe
	 * using a 3rd party Java VM that is not supporting Oracle/Sun private APIs. The memory estimates can be imprecise then (no way of
	 * detecting compressed references, alignments, etc.). Lucene still tries to use sensible defaults.
	 */
	 ...
	 * 
	 * <p>
	 * <b>Resource Usage:</b> This method internally uses a set of every object seen during traversals so it does allocate memory (it isn't
	 * side-effect free). After the method exits, this memory should be GCed.
	 * </p>
	 */
	...
	/**
	 * Same as {@link #sizeOf(Object)} but sums up all the arguments. Not an overload to prevent accidental parameter conflicts with
	 * {@link #sizeOf(Object)}.
	 */
	...
	/**
	 * Estimates a "shallow" memory usage of the given object. For arrays, this will be the memory taken by array storage (no subreferences
	 * will be followed). For objects, this will be the memory taken by the fields.
	 * 
	...
	/**
	 * Same as {@link #shallowSizeOf(Object)} but sums up all the arguments. Not an overload to prevent accidental parameter conflicts with
	...
	/**
	 * Same as {@link #shallowSizeOf(Object)} but sums up all the arguments. Duplicate objects are not resolved and will be counted twice.
	 * Not an overload to prevent accidental parameter conflicts with {@link #shallowSizeOf(Object)}.
	 */
	...
	/**
	 * Returns the shallow instance size in bytes an instance of the given class would occupy. This works with all conventional classes and
	 * primitive types, but not with arrays (the size then depends on the number of elements and varies from object to object).
	 * 
	 * @see #shallowSizeOf(Object)
	 * @throws IllegalArgumentException
	 *             if {@code clazz} is an array class.
	 */
	...
	/**
	 * Non-recursive version of object descend. This consumes more memory than recursive in-depth traversal but prevents stack overflows on
	 * long chains of objects or complex graphs (a max. recursion depth on my machine was ~5000 objects linked in a chain so not too much).
	 * 
	 * @param stack
	 *            Root objects.
	 */
	private static long measureSizeOf(ArrayList<Object> stack) {
		...
		while (!stack.isEmpty()) {
			...
			if (obClazz.isArray()) {
				/*
				 * Consider an array, possibly of primitive types. Push any of its references to the processing stack and accumulate this
				...
				/*
				 * Consider an object. Push any references it has to the processing stack and accumulate this object's shallow size.
				 */
				try {
	...
	/**
	 * This method returns the maximum representation size of an object. <code>sizeSoFar</code> is the object's size measured so far.
	 * <code>f</code> is the field being probed.
	 * 
	 * <p>
	 * The returned offset will be the maximum of whatever was measured so far and <code>f</code> field's offset and representation size
	 * (unaligned).
	 */
	private static long adjustForField(long sizeSoFar, final Field f) {
		...
		if (objectFieldOffsetMethod != null) {
			try {
				...
				throw new RuntimeException("Call to Unsafe's objectFieldOffset() throwed " + "checked Exception when accessing field " + f
						.getDeclaringClass().getName() + "#" + f.getName(), cause);
			}
		} else {
