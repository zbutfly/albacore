...
public class Loggers extends Utils {
	...
	public static void initializeLog4j() {
		...
		for (String conf : customized_configs)
			try {
				in = cl.getResourceAsStream(conf);
				if (in != null) return;
			} finally {
				if (in != null) try {
					in.close();
	...
	/**
	 *
	 * ANSIColorLayout is a Log4J Layout that formats messages using ANSI
	 * colors.
	 *
	 * Each level (DEBUG,INFO,WARN) has its own color that you can customize. To
	 * use this file, in your log4j.properties or log4j.xml specify
	 * ANSIColorLayout instead of a PatternLayout.
	 *
	 *
	 ...
	 */
	public static class ANSIColorPatternLayout extends PatternLayout {
		...
		public ANSIColorPatternLayout() {
			setDefaultColors();
		}

		public ANSIColorPatternLayout(String string) {
			super(string);
			setDefaultColors();
		}
