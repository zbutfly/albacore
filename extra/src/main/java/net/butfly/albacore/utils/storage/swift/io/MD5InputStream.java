 ...
 * MD5 algorithm is intended for digital signature applications, where a large
 * file must be "compressed" in a secure manner before being encrypted with a
...
public class MD5InputStream extends FilterInputStream {
	...
	/**
	 * Reads up to length bytes of data from this input stream into an array of
	...
	/**
	 * Get a 32-character hex representation representing hash of the stream so
	 * far.
	 * 
...
class MD5 {
	...
	private int[] decode(byte buffer[], int len, int offset) {
		int i, j;
		for (i = j = 0; j < len; i++, j += 4) {
			decodeBuffer[i] = ((buffer[j + offset] & 0xff)) | (((buffer[j + 1 + offset] & 0xff)) << 8)
					| (((buffer[j + 2 + offset] & 0xff)) << 16) | (((buffer[j + 3 + offset] & 0xff)) << 24);
		}
		return decodeBuffer;
	}
