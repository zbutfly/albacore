...
public class WrappedDataFrame<K, V> implements PairWrapped<K, V> {
	...
	final private static RowMarshaller marshaller = new RowMarshaller();
	final Class<K> kClass;
	final Class<V> vClass;
	final protected transient DataFrame frame;

	public WrappedDataFrame(DataFrame frame, Class<K> kClass, Class<V> vClass) {
	...
	public WrappedDataFrame(SQLContext ssc, RDD<V> rdd) {
		this.ssc = ssc;
	...
	@SafeVarargs
	public WrappedDataFrame(SQLContext ssc, V... t) {
		this(ssc, Arrays.asList(t));
	}

	public WrappedDataFrame(SQLContext ssc, List<V> t) {
	...
	public WrappedDataFrame<K, V> repartition(float ratio) {
		return new WrappedDataFrame<K, V>(frame.repartition((int) Math.ceil(rdd().getNumPartitions() * ratio)), kClass, vClass);
	}

	@Override
	public WrappedDataFrame<K, V> unpersist() {
		return new WrappedDataFrame<K, V>(frame.unpersist(), kClass, vClass);
	}

	@Override
	public WrappedDataFrame<K, V> persist() {
		return new WrappedDataFrame<K, V>(frame.persist(), kClass, vClass);
	}
	...
	public WrappedDataFrame<K, V> filter(Function<Tuple2<K, V>, Boolean> func) {
		return new WrappedDataFrame<>(ssc, mapWithKey(jrdd().filter(func)));
	}

	@Override
	public <K2, V2> PairWrapped<K2, V2> mapToPair(PairFunction<Tuple2<K, V>, K2, V2> func) {
		return new WrappedDataFrame<>(ssc, mapWithKey(jrdd().mapToPair(func)));
	}
