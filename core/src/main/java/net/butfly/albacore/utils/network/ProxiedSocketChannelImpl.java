 ...
 */
public class ProxiedSocketChannelImpl extends SocketChannel {
	...
	protected static final int TIMEOUT_INFINITE = 0;
	 ...
	 *
	 * @param proxy
	 *            the required proxy configuration (null == Proxy.NO_PROXY)
	 * @throws IOException
	 */
	public ProxiedSocketChannelImpl(Proxy proxy) throws SocketException {
		super(SelectorProvider.provider());
		this.socket = new Socket(new Proxy(Type.SOCKS, new InetSocketAddress(System.getProperty("socksProxyHost", "localhost"),
				Integer.parseInt(System.getProperty("socksProxyPort", "7070")))));
	 ...
	 *            the SYNCHRONIZED input stream
	 * @param dsts
	 *            the array of destination buffers
	 * @param offset
	 *            the offset for first available buffer
	 * @param length
	 ...
	 *
	 * @param output
	 *            the SYNCHRONIZED output stream
	 * @param src
	 *            the source
	 ...
	 *            the sources
	 * @param offset
	 *            the offset for the initial src buffer
	 * @param length
	 *            the number of src buffers available
	 * @return returns number of bytes written
	 * @throws IOException
	 *             in case of exceptions
	 */
