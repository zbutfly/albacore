...
abstract class MemberUtils {
	 ...
	 *
	 * When a {@code public} class has a default access superclass with {@code public} members, these
	 * members are accessible. Calling them from compiled code works fine. Unfortunately, on some JVMs,
	 * using reflection to invoke these members seems to (wrongly) prevent access even when the modifier is
	 * {@code public}. Calling {@code setAccessible(true)} solves the problem but will only work from
	...
	/**
	 * Compares the relative fitness of two sets of parameter types in terms of matching a third set of
	 * runtime parameter types, such that a list ordered by the results of the comparison would return the
	...
	/**
	 * Gets the number of steps required needed to turn the source class into the destination class. This
	 * represents the number of steps in the object hierarchy graph.
	 * 
	 * @param srcClass
	 *            The source class
	 * @param destClass
	...
	private static float getObjectTransformationCost(Class<?> srcClass, final Class<?> destClass) {
		...
		/*
		 * If the destination class is null, we've travelled all the way up to an Object match. We'll
		 * penalize this by adding 1.5 to the cost.
		 */
		if (srcClass == null) {
			cost += 1.5f;
		}
